$(document).ready(function() {
    ...
    var table = $('#descontadocomercios').DataTable({
        ...
        buttons: [{
            text: 'Exportar',
            extend: 'excelHtml5',
            title: 'Fecosva_extracto_empresas_del_' + fecha_inicio + '_al_' + fecha_fin,
            attr: {
...
$(document).on('click', '#searchdesc', function() {
    ...
    $.ajax({
        url: base_url + "Extractos/getDatos",
        method: "POST",
        contentType: false,
        processData: false,
        data: fd
...
function marcar_pagados_confirm(event) {
    ...
    if (fecha_pagado != '') {
        ...
        fd.append("fecha_fin", fecha_fin);
        fd.append("ex_cif", ex_cif);
        fd.append("fecha_pagado", fecha_pagado);
        fd.append("csrf_fecos", $('[name="csrf_fecos"]').val());

        $.ajax({
            url: base_url + "Extractos/pagarRemesa",
            method: "POST",
            contentType: false,
            processData: false,
            data: fd
