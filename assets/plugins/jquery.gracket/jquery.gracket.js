            ...
            init: function(options) {
                ...
                for (var r = 0; r < round_count; r++) {
                    ...
                    for (var g = 0; g < game_count; g++) {
                        ...
                        var
                            game_html = helpers.build.game(this.gracket.settings, r, g),
            ...
            build: {
                team: function(data, node) {
                    var html = [
                        ...
                        '&nbsp;' + data.name + '&nbsp;',
                        '<small>',
                        ((typeof data.score === "undefined") ? "" : (data.score < 1) ? "" : data.score),
                        '</small>',
                        '</h3>'
                    ].join("");
                    return team = $("<div />", {
                        ...
                        "data-user": data.id,
                        "data-inscripcion_id": (data.inscripcion_id || null)
                    });
                },
                game: function(node, r, g) {
                    var match_id = node.src[r][g][0].match_id
                    return game = $("<div />", {
                ...
                labels: function(data, offset) {
                    ...
                    for (i = 0; i < len; i++) {
                        ...
                        $("<h5 />", {
                            "html": (off.labels.length ? off.labels[i] : titleround),
                            "class": off["class"]
                        }).css({
                            "position": "absolute",
                            "left": left,
