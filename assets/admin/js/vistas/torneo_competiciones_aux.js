...
table_competiciones = $("#table_competiciones").DataTable({
    ...
    columns: [{ //0
        ...
        render: function(data, type, row) {
            ...
            if (row.modalidad == 'kata' && row.estado == 2) {
                html += '<button type="button" data-tooltip title="Ver clasificación" class="btn btn-sm btn-outline-primary" data-original-title="Clasificación"  data-clasificacion="' + row.competicion_torneo_id + '" data-modalidad="' + row.modalidad + '"><i class="fas fa-medal"></i></a>';
            }
            return html
        }
    }, ],
