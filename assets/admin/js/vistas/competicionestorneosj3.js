...
$(document).on('click', '[data-generar-tablero]', function () {
    ...
    fd.append("competicion_grupos", competicion_grupos);
    ...
    $.ajax({
        ...
        if (response.error > 0) {
            ...
            if (competicion_tipo == 'liga') {
                var html = `<h4 class="card-title d-flex justify-content-between align-items-center">Liga <span id="totalcomabtes"></span></h4>
                <div id="fasegrupos"></div>
                <hr>
                <h4>Eliminatorias</h4>
                <div id="faseeliminatorias">
                ...
                <button type="button" data-guardar-liguilla="${competicion_torneo_id}" class="btn btn-sm btn-default m-3">Guardar</button>`;
                $('#tablero-competicion').html(html);
                var totalinscritos = response.inscritos.length;
                var grupos = [];
            ...
            if (competicion_tipo == 'rey') {
                ...
                <button type="button" data-guardar-rey="${competicion_torneo_id}" class="btn btn-sm btn-default m-3">Guardar grupos</button>`;
                $('#tablero-competicion').html(html);
                var totalinscritos = response.inscritos.length;
                var grupos = [];
                var ngrupos = response.competicion_grupos;
                ...
                for (let index = 0; index < grupos.length; index++) {
                    var html = `<div class="border-bottom d-flex flex-row justify-content-start my-3" id="grupokumite_${grupos[index]}" grupo="${grupos[index]}">
                    <table class="table table-striped table-bordered text-center w-auto" id="tablakumite_${grupos[index]}" data-rondas="no">
                    <thead>
                        <tr>
                            <th colspan="4" class="bg-white text-primary font-weigth-bold">GRUPO ${grupos[index]}</th>
                        </tr>
...
function dibulareliminatorias() {
    ...
    if (grupos == 2) {
        var ronda = [
            [
                { name: "Primero G1", id: "g1|1" },
                { name: "Segundo G2", id: "g2|2" }
            ],
            [
                { name: "Primero G2", id: "g2|1" },
                { name: "Segundo G1", id: "g1|2" }
            ]
        ];
        rounds.push(ronda);
        var ronda = [
    ...
    if (grupos > 4) {
        ...
        for (let index = 0; index < n_rondas; index++) {
            ...
            if (index == 0) {
                ...
                for (let i = 0; i < matches; i++) {
                    var match = ronda[i];
                    match[0].name = 'Primero G' + primerosgrupo
                ...
                for (let i = 0; i < matches; i++) {
                    if (primerosgrupo > 0) {
                        var match = ronda[i];
                        match[1].name = 'Primero G' + primerosgrupo
...
$(document).on('click', '[data-guardar-rey]', function () {
    swal.fire({
        icon: 'question',
        title: 'Guardar orden de competición',
        html: '¿Quieres guardar los grupos y eliminatorias en el orden actual?',
        showCancelButton: true,
        confirmButtonText: 'Si, guardar',
        cancelButtonText: 'No, cerrar',
    }).then((result) => {
